...
## Live verification, completed (2026-08-08, later same day)

Reconnected the browser after the earlier blocker and finished the click-through properly, using the existing "Admin demo mode" feature (`Admin → Test Accounts`, `switch_role_for_demo()`/`switch_back_to_admin()`) rather than a second real account — exactly the pattern `TASK6.md` established for this kind of test.

**Setup found two real, pre-existing gaps** (not caused by this session's own code) that had to be fixed before any live test was even possible:
1. The Admin Demo Merchant account had no pickup location saved (`merchant_profiles.pickup_latitude/longitude` both `null`) — meaning the pre-existing "Get delivery quote" (Lalamove) button had *also* never actually worked for this account, silently, since day one. Root cause: the pickup-address page's map/GPS coordinate picker only renders when `maps.locationiq` is configured with a real key, which it isn't — the PSGC dropdown address alone never writes lat/lng. Fixed by setting real coordinates for the address already entered (same values the map picker would have written); flagged so a real LocationIQ key eventually closes this properly instead of needing a manual coordinate fix every time.
2. The same gap exists for any new `customers` row (no lat/lng from the UI either) — fixed the same way for one TEST customer, cleaned up after.

**The real bug this test caught: every user-invoked Edge Function in this codebase was silently broken when called from a real browser.** Clicking "Free distance estimate" (and, to confirm it wasn't new-code-specific, the pre-existing "Get delivery quote" button too) both failed with Supabase's generic `FunctionsFetchError` ("Failed to send a request to the Edge Function"). Root-caused via a monkey-patched `fetch`, direct `fetch()` reproduction with the app's real session token, and reading `@supabase/supabase-js`'s own source: `src/lib/supabase.js` attaches a global `x-jomhub-device-id` header (added later, for device-approval tracking) to *every* Supabase client request — but **no Edge Function's CORS `Access-Control-Allow-Headers` list included it**, so the browser's own CORS preflight silently blocked the real request before it ever reached the network tab. This affected all 21 functions under `supabase/functions/`, not just the ones built this session — meaning PayMongo/Maya/Stripe/PayPal top-up, the Lalamove delivery quote, OCR, Cloudinary/Google Drive uploads, JomBits AI, and SMS had likely never been successfully invoked from a real logged-in browser session since the device-id header was introduced, despite several of them being marked "deployed and smoke-tested" in `TASKS.md` (those tests were all `curl`, which has no CORS enforcement, and never exercised this).

**Fixed**: added `x-jomhub-device-id` to `Access-Control-Allow-Headers` in all 21 edge functions, redeployed every one preserving its original JWT-verification setting (`supabase functions list` used to confirm each function's existing `verify_jwt` value before redeploying, so none of them flipped auth mode by accident). Re-tested live: "Free distance estimate" now works completely — real OSRM road distance (14.24 km, ~19 min), a correct fee suggestion from the existing `calculate_standard_shipping()` formula, and the Leaflet map rendering both pins and the real route polyline through actual Metro Manila streets. The pre-existing Lalamove quote button should now also work for the first time, though that wasn't separately re-verified since no Lalamove credentials are configured.

**Bonus confirmation**: the same live order also proved the `purchase_orders` trigger from earlier this session — a `PO-260808-23AA85` row was generated automatically the moment the order was created, exactly as designed.

**Cleanup**: the test order, its line item, its auto-generated PO row, the wallet debit transaction, and the TEST customer were all deleted afterward (checked `Admin → Test Accounts`' one-click "Delete demo data" first, but it does a full `reset_admin_demo_data()` — wipes the persistent sample product catalog and zeroes the wallet balance, far broader than needed — so cleanup was done surgically by hand instead, verified empty afterward). The merchant pickup address/coordinates were left in place since they're real account data that was simply missing, not test data.