...
- **Not yet done**: no real browser click-through of the new "Free distance estimate" button — there were 0 real orders in production at the time of this session (clean slate), and fabricating one just to click this one button would mean seeding a wallet-funded checkout end-to-end for a low-risk, display-only feature. Recommend the next real Merchant shipping-fee-proposal on the live site doubles as this feature's first live test — if the OSRM call or the fee suggestion misbehaves, it fails open (manual entry still works, exactly as before this existed).
- **Not built this session** (flagged in the original recommendations, still true): self-hosted OSRM (the public demo server is explicitly non-production-grade per its own usage policy — fine for now, not for real volume), Leaflet route-polyline map visualization (the API already returns route geometry, just nothing renders it yet), and re-litigating the literal module/service-boundary question.