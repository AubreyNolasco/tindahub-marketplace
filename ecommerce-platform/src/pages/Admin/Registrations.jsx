  ...
  return (
    <div className="mx-auto max-w-6xl p-4 sm:p-6 lg:p-8">
      ...
      <section className="mb-7 grid gap-4 lg:grid-cols-[1fr_1.1fr]">
        ...
        <div className="card p-5"><h2 className="font-display font-bold">Upcoming unavailable schedules</h2><div className="mt-3 max-h-72 space-y-2 overflow-y-auto">{unavailable.length === 0 ? <p className="rounded-xl bg-surface-inset p-4 text-sm text-ink/45">No blocked schedules.</p> : unavailable.map((slot) => <div key={slot.id} className="flex items-center justify-between gap-3 rounded-xl border border-black/5 p-3"><div><p className="text-sm font-bold">{new Date(`${slot.unavailable_date}T00:00:00`).toLocaleDateString('en-PH', { dateStyle: 'medium' })} · {slot.unavailable_time ? timeLabel(slot.unavailable_time) : 'Whole day'}</p><p className="mt-0.5 text-xs text-ink/45">{slot.reason}</p></div><button onClick={() => removeUnavailable(slot)} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl text-coral-600 hover:bg-coral-100" title="Make available again"><Trash2 size={17} /></button></div>)}</div></div>
      </section>
