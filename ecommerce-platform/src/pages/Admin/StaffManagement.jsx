...
const mapRpcError = (message = '') => {
  const map = {
    ADMIN_REQUIRED: 'Only an admin can manage staff access.',
    INVALID_EMAIL: 'Please enter a valid staff email.',
    INVALID_NAME: 'Full name must be between 2 and 120 characters.',
    NO_PERMISSIONS: 'Select at least one staff permission.',
    STAFF_NOT_FOUND: 'No staff record found for this account.'
  }
...
function PermissionGrid({ value, onChange, disabled }) {
  ...
  return <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{ADMIN_PERMISSIONS.map(([key, label]) => <label key={key} className={`flex items-center gap-3 rounded-xl border p-3 text-sm font-semibold ${value.includes(key) ? 'border-teal-300 bg-teal-50 text-teal-900' : 'border-black/[0.07] bg-surface text-ink/55'}`}><input type="checkbox" disabled={disabled} checked={value.includes(key)} onChange={() => toggle(key)} className="h-4 w-4 accent-teal-600" />{label}</label>)}</div>
}
...
useEffect(() => { load() }, [])
  ...
  const invite = async (event) => {
    ...
    setSaving(false)
    if (error) return toast.error(mapRpcError(error.message))
    setForm(emptyForm)
...
const updateStaff = async (entry, changes) => {
    ...
    if ('permissions' in changes) {
      ({ error } = await supabase.rpc('update_staff_access', { p_user_id: entry.user_id, p_permissions: changes.permissions }))
    } else if ('active' in changes) {
      ({ error } = await supabase.rpc('set_staff_active', { p_user_id: entry.user_id, p_active: changes.active }))
    }
    if (error) return toast.error(mapRpcError(error.message))
  ...
  const revoke = async (invite) => {
    const { error } = await supabase.rpc('revoke_staff_invitation', { p_invite_id: invite.id })
    if (error) return toast.error(mapRpcError(error.message))
...
<form onSubmit={invite} className="mt-6 rounded-3xl border border-black/[0.06] bg-surface p-5 shadow-card sm:p-7"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-teal-700 text-white"><MailPlus size={21} /></span><div><h2 className="font-display text-lg font-bold text-ink">Create staff access</h2><p className="text-xs text-ink/45">Staff will use the regular secure email sign-in link.</p></div></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold text-ink/65">Full name<input required value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} className="input-field mt-1.5" /></label><label className="text-sm font-semibold text-ink/65">Staff email<input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="input-field mt-1.5" /></label></div><p className="mb-2 mt-5 text-xs font-bold uppercase tracking-wide text-ink/40">Allowed modules</p><PermissionGrid value={form.permissions} onChange={(permissions) => setForm({ ...form, permissions })} /><button disabled={saving} className="btn-primary mt-5 flex items-center gap-2">{saving ? <Loader2 size={16} className="animate-spin" /> : <UserCog size={16} />} Create staff access</button></form>
    {loading ? <div className="flex justify-center py-16"><Spinner /></div> : <><section className="mt-8"><h2 className="font-display text-lg font-bold text-ink">Active staff</h2><div className="mt-3 space-y-4">{staff.length === 0 && <p className="rounded-2xl bg-surface p-5 text-sm text-ink/45">No staff has accepted an invitation yet.</p>}{staff.map((entry) => <StaffCard key={entry.user_id} entry={entry} onSave={updateStaff} />)}</div></section><section className="mt-8"><h2 className="font-display text-lg font-bold text-ink">Invitations</h2><div className="mt-3 grid gap-3 sm:grid-cols-2">{invitations.map((invite) => <div key={invite.id} className="card p-4"><div className="flex items-center justify-between gap-3"><div><p className="font-semibold text-ink">{invite.full_name}</p><p className="text-xs text-ink/45">{invite.email}</p></div><span className={`badge ${invite.status === 'accepted' ? 'bg-teal-100 text-teal-700' : invite.status === 'revoked' ? 'bg-coral-100 text-coral-600' : 'bg-mango-100 text-mango-700'}`}>{invite.status}</span></div>{invite.status === 'pending' && <button onClick={() => revoke(invite)} className="btn-secondary mt-3 flex w-full items-center justify-center gap-2 text-sm"><UserX size={15} /> Revoke invitation</button>}</div>)}</div></section></>}
  </div>
}
