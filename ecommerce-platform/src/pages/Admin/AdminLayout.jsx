...
function Sidebar({ open, collapsed, onClose, onToggleCollapse, visibleSections }) {
  return (
    <>
      ...
      <aside className={`fixed bottom-0 top-16 z-50 flex flex-col border-r border-black/10 bg-white text-ink shadow-2xl transition-all duration-300 lg:sticky lg:z-20 lg:h-[calc(100vh-4rem)] lg:shrink-0 ${open ? 'left-0' : '-left-72'} ${collapsed ? 'lg:w-[84px]' : 'lg:w-64'} w-72 lg:left-0`}>
        <div className={`flex h-[76px] items-center border-b border-black/5 bg-teal-50 ${collapsed ? 'justify-center px-3' : 'justify-between px-5'}`}>
          {!collapsed && <div className="min-w-0"><div className="flex items-center gap-2 font-display font-bold text-teal-900"><ShieldCheck size={20} className="text-mango-600" /> Admin Center</div><p className="mt-1 text-xs text-ink/55">Manage RM Hub</p></div>}
