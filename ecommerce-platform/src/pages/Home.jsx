  ...
  return <div className="overflow-hidden bg-cream">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-4 py-12 sm:px-6 sm:py-16 lg:grid-cols-[1fr_1.02fr] lg:gap-16 lg:py-24">
        <div>
          ...
          <div className="mt-5 inline-flex max-w-full items-center gap-3 rounded-2xl border border-teal-100 bg-white/80 px-4 py-3 shadow-sm backdrop-blur">
            ...
            <div><p className="text-sm font-bold text-ink">Fully secured by your email</p><p className="text-xs leading-5 text-ink/50">Passwordless, one-time sign-in links help protect every account.</p></div>
          </div>
    ...
    {content.sections?.subscription !== false && <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20" id="subscribe">
      <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-center"><div><span className="inline-flex items-center gap-2 rounded-full bg-teal-50 px-3 py-1.5 text-xs font-bold text-teal-700"><Store size={14} /> For Merchants</span><h2 className="mt-4 font-display text-3xl font-bold text-ink sm:text-4xl">How to subscribe and open your store</h2><p className="mt-4 leading-7 text-ink/60">Create a Merchant profile, choose a subscription, and submit a screenshot of your payment. The Admin reviews the application before activating your store.</p>
        <div className="mt-6 space-y-3">{['Create a Merchant account and enter your business details.', 'Choose 6 months, 1 year, or 2 years.', 'Select GCash, Maya, or bank transfer and upload the screenshot.', 'Wait for Admin approval, then manage your products and orders.'].map((text, index) => <div key={text} className="flex gap-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-teal-600 text-xs font-bold text-white">{index + 1}</span><p className="pt-0.5 text-sm leading-6 text-ink/65">{text}</p></div>)}</div>
    ...
    {content.sections?.topup !== false && <section className="border-y border-black/5 bg-white py-16 sm:py-20" id="topup"><div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:items-center"><div className="rounded-[1.75rem] bg-mango-100/60 p-6 sm:p-8"><div className="grid gap-3 sm:grid-cols-2">{[{ icon: CircleDollarSign, title: 'Enter amount', text: 'Choose how much to add to your wallet.' }, { icon: FileImage, title: 'Upload screenshot', text: 'Submit payment proof and reference number.' }, { icon: Clock3, title: 'Admin review', text: 'The request stays pending while it is checked.' }, { icon: Wallet, title: 'Wallet credit', text: 'Approved funds appear in the wallet balance.' }].map(({ icon: Icon, title, text }) => <div key={title} className="rounded-2xl bg-white p-5 shadow-sm"><Icon size={20} className="text-mango-600" /><h3 className="mt-3 font-bold text-ink">{title}</h3><p className="mt-1 text-xs leading-5 text-ink/55">{text}</p></div>)}</div></div>
