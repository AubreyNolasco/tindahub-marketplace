  ...
  return <div className="overflow-hidden bg-bg">
    ...
    <div id="how-it-works" className="scroll-mt-24">
      <section className="mx-auto max-w-7xl px-4 pt-16 sm:px-6 sm:pt-20">
        ...
        <h2 className="mt-2 font-display text-3xl font-bold text-ink sm:text-4xl">A clearer path from signup to payment.</h2>
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-2xl">
            ...
            <h3 className="mt-2 font-display text-2xl font-semibold tracking-[-0.02em] text-ink sm:text-3xl">Built for every role in the ecosystem.</h3>
          </div>
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {categoryCards.map(({ title, description, icon: Icon, href }) => (
            <Link key={title} to={href} className="group rounded-[1.5rem] border border-black/[0.06] bg-surface p-5 shadow-sm transition hover:-translate-y-1 hover:border-teal-200 hover:shadow-md">
              ...
              <h4 className="mt-4 font-display text-lg font-semibold text-ink">{title}</h4>
      ...
      {content.sections?.benefits !== false && <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20">
        <div className="mx-auto max-w-3xl text-center"><p className="text-xs font-bold uppercase tracking-[0.2em] text-teal-600">Built for real business</p><h3 className="mt-3 font-display text-3xl font-bold text-ink sm:text-4xl">Everything you need to buy, sell, and grow with confidence.</h3><p className="mt-4 text-ink/60">JOM HUB replaces scattered chats, manual records, and disconnected payment tracking — all in one organized workspace.</p></div>
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{benefits.map(({ icon: Icon, title, text }) => <div key={title} className="group rounded-2xl border border-black/[0.06] bg-surface p-5 shadow-card transition hover:-translate-y-1 hover:border-teal-100 hover:shadow-soft"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-teal-50 text-teal-700 transition group-hover:bg-teal-600 group-hover:text-white"><Icon size={19} /></span><h4 className="mt-4 font-display text-base font-bold text-ink sm:text-lg">{title}</h4><p className="mt-2 text-sm leading-6 text-ink/60">{text}</p></div>)}</div>
      </section>}

      {content.sections?.process !== false && <section className="bg-teal-950 py-16 text-white sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-mango-300">How JOM HUB works</p><h3 className="mt-3 font-display text-3xl font-bold sm:text-4xl">A clearer path from discovery to payment.</h3></div><Link to="/catalog" className="inline-flex items-center gap-2 text-sm font-semibold text-mango-300">View the marketplace <ArrowRight size={16} /></Link></div>
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{transactionSteps.map(({ icon: Icon, title, text }, index) => <div key={title} className="relative rounded-2xl border border-white/10 bg-white/[0.07] p-5"><span className="absolute right-4 top-4 font-display text-3xl font-bold text-white/[0.08]">0{index + 1}</span><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-mango-500 text-ink"><Icon size={21} /></span><h4 className="mt-5 text-lg font-bold">{title}</h4><p className="mt-2 text-sm leading-6 text-white/60">{text}</p></div>)}</div>
        </div>
      </section>}
    ...
    {/* ===== FOR MERCHANTS ===== */}
    {content.sections?.subscription !== false && <section id="for-merchants" className="mx-auto max-w-7xl scroll-mt-24 px-4 py-16 sm:px-6 sm:py-20">
    ...
    {/* ===== FOR RESELLERS ===== */}
    {content.sections?.topup !== false && <section id="for-resellers" className="scroll-mt-24 py-16 sm:py-20">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:items-center">
        <div className="rounded-[1.75rem] bg-mango-100/60 p-6 dark:bg-mango-500/10 sm:p-8"><div className="grid gap-3 sm:grid-cols-2">{[{ icon: CircleDollarSign, title: 'Scan and pay', text: 'Use the JOM HUB InstaPay QR to pay AUBREY NOLASCO and enter the amount.' }, { icon: FileImage, title: 'Upload proof', text: 'Upload the screenshot along with the one-use reference number.' }, { icon: Clock3, title: 'Admin verification', text: 'Admin matches the actual payment; duplicate references are blocked.' }, { icon: Wallet, title: 'Wallet credit', text: 'Approved funds appear in your wallet balance.' }].map(({ icon: Icon, title, text }) => <div key={title} className="rounded-2xl bg-surface p-5 shadow-sm"><Icon size={20} className="text-mango-600" /><h4 className="mt-3 font-bold text-ink">{title}</h4><p className="mt-1 text-xs leading-5 text-ink/55">{text}</p></div>)}</div></div>
    ...
    <div id="services" className="scroll-mt-24">
      <section className="mx-auto max-w-7xl px-4 pt-16 sm:px-6 sm:pt-20">
        ...
        <h2 className="mt-2 font-display text-3xl font-bold text-ink sm:text-4xl">Referral programs and delivery partners.</h2>
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
        <div className="rounded-[2rem] border border-black/[0.06] bg-white/80 p-6 shadow-card backdrop-blur-xl dark:border-white/10 dark:bg-surface/80 sm:p-8">
          <div className="max-w-2xl">
            ...
            <h3 className="mt-2 font-display text-2xl font-semibold tracking-[-0.02em] text-ink sm:text-3xl">Premium tools for your next growth step.</h3>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20">
        <div className="grid lg:grid-cols-[1.05fr_0.95fr] rounded-[2rem] border border-teal-100 bg-surface shadow-2xl shadow-teal-900/[0.06] overflow-hidden">
          <div className="relative p-6 sm:p-10 lg:p-14">
            ...
            <h3 className="mt-5 max-w-xl font-display text-3xl font-extrabold leading-tight text-ink sm:text-4xl">Earn by referring customers to a clinic.</h3>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl border border-teal-100 bg-teal-50 p-4 dark:border-teal-800 dark:bg-teal-500/10"><Stethoscope size={20} className="text-teal-700" /><h4 className="mt-3 text-sm font-bold text-ink">Dental & Optical Clinics</h4><p className="mt-1 text-xs leading-5 text-ink/50">Partner clinics offering dental cleaning, eye checkups, and more.</p></div>
              <div className="rounded-2xl border border-mango-200 bg-mango-100/60 p-4 dark:border-mango-700 dark:bg-mango-500/10"><TrendingUp size={20} className="text-mango-700" /><h4 className="mt-3 text-sm font-bold text-ink">Referral Fee</h4><p className="mt-1 text-xs leading-5 text-ink/50">Earn a referral fee that is automatically transferred to your wallet.</p></div>
            </div>
        ...
        <div className="relative overflow-hidden rounded-[2rem] border border-teal-100 bg-surface shadow-2xl shadow-teal-900/[0.06]">
          <div className="grid lg:grid-cols-[1fr_1fr]">
            ...
            <div className="p-6 sm:p-10 lg:p-14">
              ...
              <h3 className="mt-5 max-w-xl font-display text-3xl font-extrabold leading-tight text-ink sm:text-4xl">Faster shipping with Lalamove.</h3>
    ...
    {/* ===== TESTIMONIALS ===== */}
    <section id="testimonials" className="scroll-mt-24 bg-[linear-gradient(180deg,#EDF7F1_0%,#FFFFFF_48%,#F7FAF7_100%)] py-16 dark:bg-none dark:bg-bg sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
    ...
    {/* ===== FAQ ===== */}
    <section id="faq" className="scroll-mt-24 bg-[linear-gradient(180deg,#FFFFFF_0%,#EDF7F1_100%)] py-16 dark:bg-none dark:bg-bg sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="mx-auto max-w-3xl text-center">
