...
function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="h-5 w-5 shrink-0">
      ...
      <path fill="#34A853" d="M12 22c2.7 0 4.98-.9 6.64-2.36L15.4 17.1c-.9.6-2.04.96-3.4.96-2.61 0-4.82-1.76-5.61-4.13H3.04v2.62A10 10 0 0 0 12 22Z"/>
      <path fill="#FBBC05" d="M6.39 13.93A6.02 6.02 0 0 1 6.08 12c0-.67.12-1.32.31-1.93V7.45H3.04A10 10 0 0 0 2 12c0 1.64.39 3.2 1.04 4.55l3.35-2.62Z"/>
...
export default function Login() {
  const { user, profile, profileError, loading, signInWithGoogle, requestEmailOtp, verifyEmailOtp, signOut } = useAuth()
  ...
  const mainCard = (
    <div className="rounded-3xl border border-black/[0.06] bg-surface p-6 shadow-xl shadow-teal-900/[0.06] sm:p-8">
      ...
      <h1 className="mt-4 font-display text-3xl font-bold text-ink">Continue to JOM HUB</h1>
      <p className="mt-2 text-sm leading-6 text-ink/55">
        {otpSent ? `Enter the 6-digit code sent to ${testEmail}.` : 'Enter your email and we will send a secure 6-digit verification code.'}
      </p>
      {otpSent ? (
        <form onSubmit={submitOtp} className="mt-7 space-y-3">
          <input
            ...
            placeholder="000000"
            className="input-field text-center font-display text-2xl font-bold tracking-[0.35em]"
          />
          <button disabled={redirecting || otp.length !== 6} className="btn-primary flex w-full items-center justify-center gap-2">
            {redirecting && <Loader2 size={18} className="animate-spin" />}Verify code
          </button>
          <button type="button" disabled={redirecting || emailCooldown > 0} onClick={submitEmailLogin} className="btn-secondary w-full">
            {emailCooldown > 0 ? `Send again in ${emailCooldown}s` : 'Resend code'}
          </button>
          <button type="button" onClick={() => { setOtpSent(false); setOtp('') }} className="w-full py-2 text-xs font-bold text-teal-700">
        ...
        <form onSubmit={submitEmailLogin} className="mt-7 space-y-3">
          ...
          <button disabled={redirecting} className="btn-primary flex w-full items-center justify-center gap-2">
            {redirecting ? <Loader2 size={18} className="animate-spin" /> : <MailCheck size={18} />}
  ...
  const guideCard = (
    <section className="rounded-3xl border border-teal-100 bg-surface p-5 shadow-card" aria-labelledby="login-guide-title">
      ...
      <ol className="mt-4 space-y-3">
        {loginSteps.map(({ icon: Icon, title, text }, index) => (
          <li key={title} className="flex gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-ink">
                <span className="mr-1 text-teal-600">{index + 1}.</span>
                {title}
              </p>
              <p className="mt-0.5 text-xs leading-5 text-ink/50">{text}</p>
            </div>
          </li>
        ))}
      </ol>
