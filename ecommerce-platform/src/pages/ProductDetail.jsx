...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!error && data) { const [discounts, campaignProducts] = await Promise.all([getActiveCampaignDiscounts(), getActiveCampaignProducts()]); setProduct(applyCampaignDiscount(data, discounts, campaignProducts)); setQty(data.min_order_qty || 1); setCustomerSellingPrice(getSuggestedCustomerPrice(data)) }
      await loadReviews()
      setLoading(false)
    }
    load()
