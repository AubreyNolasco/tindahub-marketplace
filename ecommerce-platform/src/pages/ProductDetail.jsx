  ...
  return <div className="min-h-screen bg-[#f5f8f5] py-6 sm:py-10"><div className="mx-auto max-w-6xl px-4 sm:px-6">
    ...
    <section className="grid overflow-hidden rounded-3xl border border-black/[0.06] bg-surface shadow-soft md:grid-cols-2">
      ...
      <div className="flex flex-col p-5 sm:p-8 lg:p-10"><div className="flex flex-wrap items-center gap-3"><Link to={`/merchant-store/${product.merchant_id}`} className="flex items-center gap-1.5 text-sm font-semibold text-teal-700 hover:text-teal-900"><Store size={15} />{product.merchant_profiles?.business_name}</Link>{user && user.id !== product.merchant_id && (role === 'reseller' || role === 'merchant') && <Link to={`/${role}/chats/${product.merchant_id}`} className="flex items-center gap-1.5 text-sm text-ink/45 hover:text-teal-600"><MessageCircle size={15} /> Message store</Link>}</div>
        ...
        <div className="mt-6 border-y border-black/[0.06] py-5"><p className="text-xs font-bold uppercase tracking-wide text-ink/40">{role === 'reseller' ? 'Your reseller buying price' : 'Retail price'}</p><p className="mt-1 font-display text-3xl font-bold text-teal-700">{peso(role === 'reseller' ? getResellerUnitPrice(product, qty) : product.price)} {role === 'reseller' && <span className="text-sm font-medium text-ink/40">each at {qty} item{qty === 1 ? '' : 's'}</span>}</p>{role !== 'reseller' && product.wholesale_price && <p className="mt-1 text-sm text-ink/45">Reseller wholesale price available from {peso(product.wholesale_price)}</p>}</div>
        ...
        {product.campaign_discount_percent > 0 ? <div className="mt-4 rounded-2xl border border-mango-300 bg-mango-100 p-4 dark:border-mango-700 dark:bg-mango-500/10"><p className="text-xs font-bold uppercase tracking-wide text-mango-600">Marketplace campaign</p><div className="mt-2 flex items-center justify-between"><span className="font-display text-xl font-bold text-ink">{product.campaign_discount_percent}% OFF</span><span className="font-bold text-teal-700">{peso(getUnitPrice(product, qty))} each</span></div><p className="mt-2 text-xs text-ink/50">Campaign price applies to every quantity. Store quantity discounts are paused.</p></div> : product.discount_tiers?.length > 0 && <div className="mt-4 rounded-2xl border border-mango-300/50 bg-mango-100/40 p-4 dark:border-mango-700/50 dark:bg-mango-500/10"><p className="text-xs font-bold uppercase tracking-wide text-mango-600">Reseller quantity discounts</p><div className="mt-2 space-y-2">{product.discount_tiers.map((tier) => { const percent = Number(tier.discount_percent) || Number(((1 - Number(tier.price) / Number(product.price)) * 100).toFixed(2)); return <div key={tier.min_qty} className="flex items-center justify-between gap-3 text-sm"><span className="text-ink/60">Buy {tier.min_qty}+ items</span><span className="text-right"><strong className="text-mango-600">{percent}% OFF</strong><span className="ml-2 font-bold text-teal-700">{peso(tier.price)} each</span></span></div> })}</div></div>}
