...
// caller, so there's no merchant picker here -- a merchant can only ever
// create vouchers for their own store/products (20260806002100_voucher_write_rpcs.sql).

const SCOPES = [
  { value: 'merchant', label: 'My whole store' },
  { value: 'product', label: 'One of my products' },
  { value: 'category', label: 'One category (my products only)' },
  { value: 'shipping', label: 'Shipping discount' }
]
const initialForm = {
  code: '', scope: 'merchant', discount_type: 'percent', discount_value: '', max_discount_amount: '',
...
export default function MerchantVouchers() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const [voucherResult, productResult, categoryResult] = await Promise.all([
      supabase.from('vouchers').select('*, products(name), categories(name), voucher_redemptions(count)').order('created_at', { ascending: false }),
      supabase.from('products').select('id, name').eq('merchant_id', user.id).order('name'),
      supabase.from('categories').select('id, name').order('name')
    ])
    if (voucherResult.error) toast.error(voucherResult.error.message)
  ...
  const scopeLabel = (v) => v.scope === 'merchant' ? 'Whole store' : v.scope === 'product' ? v.products?.name : v.scope === 'category' ? v.categories?.name : 'Shipping'

  if (loading) return <div className="flex justify-center py-24"><Spinner /></div>

  return <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
    <div className="mb-6"><p className="text-xs font-bold uppercase tracking-[0.16em] text-teal-600">Grow your sales</p><h1 className="mt-1 font-display text-2xl font-bold text-ink">Vouchers</h1><p className="mt-1 max-w-2xl text-sm leading-6 text-ink/50">Create discount codes for your store, a specific product, a category, or shipping. Shoppers apply the code at checkout.</p></div>
    ...
    <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
      <form onSubmit={create} className="h-fit rounded-2xl border border-black/[0.06] bg-surface p-5 shadow-soft">
        ...
        <div className="mt-5 space-y-4">
          <label className="block text-sm font-medium text-ink/65">Code<input required maxLength="40" className="input-field mt-1 uppercase" value={form.code} onChange={update('code')} placeholder="MYSTORE10" /></label>
          <label className="block text-sm font-medium text-ink/65">Applies to
          ...
          {form.scope === 'product' && (
            <label className="block text-sm font-medium text-ink/65">Product
              <select required className="input-field mt-1" value={form.product_id} onChange={update('product_id')}>
    ...
    {editing && <div className="fixed inset-0 z-[80] overflow-y-auto bg-scrim/55 p-3 sm:p-6">
      <form onSubmit={saveEdit} className="mx-auto max-w-lg rounded-2xl bg-surface p-5 shadow-2xl sm:p-7">
        ...
        <p className="mt-1 text-xs text-ink/45">Scope, code, and target (product/category) can't be changed after creation — create a new voucher instead if those need to change.</p>
