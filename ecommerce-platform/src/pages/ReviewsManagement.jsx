...
export default function ReviewsManagement({ admin = false }) {
  ...
  const remove = (id) => { if (admin) setDeleteTarget(id) }
  const confirmRemove = async () => {
    if (!deleteTarget) return
    setDeleting(true)
    const { error } = await supabase.from('product_reviews').delete().eq('id', deleteTarget)
    setDeleting(false)
