...
export async function reverseOpenStreetMap(latitude, longitude, signal) {
  const key = `reverse:${Number(latitude).toFixed(6)},${Number(longitude).toFixed(6)}`
  const existing = cached(key)
  if (existing !== undefined) return existing
  const url = `${API_URL}/reverse?format=jsonv2&addressdetails=1&zoom=18&lat=${encodeURIComponent(latitude)}&lon=${encodeURIComponent(longitude)}`
