...
export default function Footer() {
  return (
    <footer className="mt-16 overflow-hidden bg-teal-950 text-white">
      <div className="border-b border-white/10 bg-teal-900/50"><div className="mx-auto flex max-w-7xl flex-col gap-5 px-4 py-7 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><p className="font-display text-xl font-bold">Ready to grow with RM HUB?</p><p className="mt-1 text-sm text-white/55">Create your professional Merchant or Reseller workspace today.</p></div><Link to="/signup" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-mango-500 px-5 py-3 text-sm font-bold text-ink transition hover:bg-mango-600">Join the marketplace <ArrowRight size={16} /></Link></div></div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
        <div>
          <Link to="/" className="inline-block rounded-xl bg-white px-2"><img src="/rmhub-logo.svg" alt="RM HUB" className="h-14 w-auto" /></Link>
        ...
        <div>
          <p className="font-semibold text-white text-sm mb-4">Why RM HUB</p>
      ...
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5 text-xs text-white/35 text-center">
          © {new Date().getFullYear()} RM Hub. Built for growing businesses.
        </div>
      </div>
    </footer>
  )
