...
export default function JomBits({ publicMode = false }) {
  ...
  return <>
    <button type="button" onClick={() => setOpen((value) => !value)} className="fixed bottom-4 right-4 z-[70] grid h-14 w-14 place-items-center rounded-full bg-teal-700 text-white shadow-2xl shadow-teal-950/30 transition hover:-translate-y-0.5 hover:bg-teal-800 sm:bottom-5 sm:right-5 sm:h-16 sm:w-16" aria-label={open ? 'Close JOM Bits assistant' : 'Open JOM Bits assistant'} aria-expanded={open}><Bot size={26}/><span className="absolute right-0 top-0 h-3.5 w-3.5 rounded-full border-2 border-white bg-mango-400"/></button>
    {open && <section className="fixed bottom-20 right-3 z-[69] flex h-[min(620px,calc(100dvh-6.5rem))] w-[calc(100vw-1.5rem)] flex-col overflow-hidden rounded-3xl border border-black/10 bg-white shadow-2xl shadow-ink/25 sm:bottom-24 sm:right-5 sm:w-[390px]" role="dialog" aria-labelledby="jom-bits-title">
