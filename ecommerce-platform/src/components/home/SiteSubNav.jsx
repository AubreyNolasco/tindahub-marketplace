...
export default function SiteSubNav() {
  return (
    <nav className="sticky top-16 z-30 border-b border-black/[0.05] bg-[#f7faf7]/90 backdrop-blur-xl dark:border-white/10 dark:bg-[#07120d]/90">
      <div className="mx-auto flex max-w-7xl gap-1.5 overflow-x-auto px-3 py-2 sm:px-6 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {LINKS.map(({ to, label, icon: Icon, end }) => (
          <NavLink
            ...
            className={({ isActive }) => `flex shrink-0 items-center gap-1.5 rounded-full px-3.5 py-2 text-xs font-bold whitespace-nowrap transition ${isActive ? 'bg-teal-700 text-white' : 'text-ink/60 hover:bg-teal-50 hover:text-teal-700'}`}
          >
            <Icon size={14} /> {label}
          </NavLink>
        ))}
