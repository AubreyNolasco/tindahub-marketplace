  ...
  return <>
    <Link to={`/product/${product.id}`} className="group flex h-full min-w-0 flex-col overflow-hidden rounded-xl border border-black/[0.06] bg-surface shadow-[0_2px_12px_rgba(20,32,25,0.04)] transition duration-300 hover:-translate-y-1 hover:border-teal-100 hover:shadow-soft sm:rounded-2xl">
      <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-teal-50 to-[#f8fbf8]">
        ...
        {bestDiscount > 0 && <span className="absolute bottom-1 right-1 rounded-full bg-mango-500 px-1.5 py-0.5 text-[7px] font-bold text-ink shadow-sm sm:bottom-auto sm:right-2.5 sm:top-2.5 sm:px-2.5 sm:py-1 sm:text-[10px]">{product.campaign_discount_percent ? <span className="hidden sm:inline">CAMPAIGN </span> : <span className="hidden sm:inline">Up to </span>}{Math.round(bestDiscount)}% OFF</span>}
      </div>
      <div className="flex flex-1 flex-col p-1.5 sm:p-4">
        ...
        <div className="mt-auto flex items-end justify-between gap-1 pt-1.5 sm:gap-2 sm:pt-4">
          <div className="min-w-0"><span className="block truncate font-display text-[10px] font-bold text-teal-700 sm:text-lg">{peso(role === 'reseller' ? getResellerProfitEstimate(product, Math.max(1, product.min_order_qty || 1)).buyingUnitPrice : product.price)}</span><p className="mt-1 hidden items-center gap-1 text-xs text-ink/40 sm:flex"><PackageCheck size={12} />{inStock ? `${product.stock_quantity} available` : 'Unavailable'}</p></div>
