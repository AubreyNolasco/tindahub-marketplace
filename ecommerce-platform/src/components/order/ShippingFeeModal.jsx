...
export default function ShippingFeeModal({ order, open, onClose, onSaved }) {
  ...
  // Auto-book only fires if the accepted fee carries a quotation id through
  // (see propose_order_shipping_fee / trg_notify_lalamove_dispatch_ready),
  // so manually editing the fee after quoting clears it deliberately.
  const getQuote = async () => {
    setQuoting(true)
    const { data, error } = await supabase.functions.invoke('lalamove-quote', { body: { order_id: order.id } })
    setQuoting(false)
    if (error || data?.error) {
      const message = data?.error || error?.message || 'Could not get a Lalamove quote.'
      return toast.error(message === 'LALAMOVE_NOT_CONNECTED' ? "This reseller hasn't connected Lalamove yet." : message === 'MERCHANT_PICKUP_LOCATION_MISSING' ? 'Set your pickup location before getting a Lalamove quote.' : message === 'CUSTOMER_LOCATION_MISSING' ? "This customer doesn't have a pinned location yet." : message)
    }
    setFee(data.price)
    setQuotationId(data.quotation_id)
    toast.success(`Lalamove quote: ${peso(data.price)}`)
  }
  ...
  const submit = async (event) => {
    ...
    setSaving(true)
    const { error } = await supabase.rpc('propose_order_shipping_fee', { p_order_id: order.id, p_fee: Number(fee), p_note: note || null, p_lalamove_quotation_id: quotationId })
    setSaving(false)
  ...
  return <div className="fixed inset-0 z-[90] grid place-items-center bg-scrim/60 p-4"><form onSubmit={submit} className="card w-full max-w-md p-6">
    ...
    <button type="button" disabled={quoting} onClick={getQuote} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl border border-teal-200 bg-teal-50 py-2.5 text-sm font-semibold text-teal-700 disabled:opacity-60">
      {quoting ? <Loader2 size={16} className="animate-spin"/> : <Truck size={16}/>}Get Lalamove quote
    </button>
    <label className="mt-4 block text-sm font-semibold">Actual shipping fee<input required type="number" min="0" step=".01" value={fee} onChange={(event)=>{setFee(event.target.value); setQuotationId(null)}} className="input-field mt-1" placeholder="0.00" /></label>
    {quotationId && <p className="mt-1.5 text-xs font-medium text-teal-700">Lalamove quote applied — booking will be automatic once the Reseller accepts.</p>}
