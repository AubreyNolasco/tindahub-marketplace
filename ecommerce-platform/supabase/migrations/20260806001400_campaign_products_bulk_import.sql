-- Phase 5/6 of TASK6.md: bulk import needs a stock_allocation field per
-- submission (imported CSV rows carry it); additive nullable column, no
-- existing row is affected.
alter table public.campaign_products add column if not exists stock_allocation integer check (stock_allocation is null or stock_allocation > 0);

-- Re-create submit_campaign_product with an optional stock allocation
-- argument. Same validation body as before, plus one more check.
drop function if exists public.submit_campaign_product(uuid, uuid, numeric);
