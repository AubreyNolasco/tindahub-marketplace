-- Phase 10 (cont.) + Phase 9: wires the voucher engine into the same two
-- checkout RPCs that campaign pricing was wired into
-- (20260806001700_checkout_campaign_pricing.sql) -- same principle:
-- quote_order() previews it, place_order() re-validates it for real
-- (a code that expired or hit its usage limit between quote and place
-- must be caught, not silently honored) and records the redemption.
-- p_voucher_code is optional and defaults to null on both functions, so
-- every existing caller that doesn't pass it keeps working unchanged.

alter table public.orders add column if not exists voucher_discount numeric(12,2) not null default 0;
alter table public.orders add column if not exists voucher_id uuid references public.vouchers(id);

create or replace function public.quote_order(p_merchant_id uuid, p_items jsonb, p_shipping_fee numeric default 0, p_voucher_code text default null)
...
declare
  ...
  v_voucher record; v_voucher_valid boolean := false; v_voucher_discount numeric(12,2) := 0; v_shipping_fee numeric(12,2) := coalesce(p_shipping_fee, 0); v_voucher_error text;
begin
  ...
  return jsonb_build_object(
    'subtotal', subtotal, 'shipping_fee', v_shipping_fee, 'reseller_fee', reseller_fee, 'merchant_fee', merchant_fee,
