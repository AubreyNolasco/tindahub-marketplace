...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // The adapter re-quotes and places the order in one call (Lalamove
    // quotes expire quickly, so it always quotes immediately before
    // booking rather than reusing an older one) — this is one quote
    // request + one place-order request to Lalamove, same as before.
    const booked = await getAdapter('lalamove').createBooking({ orderId: order.id, pickup, dropoff }, cred)
    if (!booked.ok || !booked.externalOrderId) throw new Error(booked.error?.code || 'LALAMOVE_BOOKING_FAILED')
    ...
    return new Response(JSON.stringify({ ok: true, lalamove_order_id: booked.externalOrderId }), { headers: { ...cors, 'Content-Type': 'application/json' } })
  } catch (error) {
    if (bookingId) {
