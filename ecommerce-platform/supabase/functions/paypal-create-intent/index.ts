...
// Reseller/Merchant-invoked from TopupModal.jsx's "Pay with PayPal" option.
// Mirrors paymongo-create-intent/index.ts's shape exactly — the PayPal
// adapter creates an Orders v2 order (not yet captured); the buyer
// approves on PayPal's page, then paypal-webhook captures it on
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const intent = await getAdapter('payments.paypal').createIntent(
    ...
    if (!intent.ok) {
      await admin.rpc('log_integration_event', {
        ...
        p_event_type: 'create_order',
        p_status: 'error',
        p_payload: { owner_id: user.id, amount: numericAmount, reference },
        p_error_message: intent.error?.message || 'Unknown error',
      })
      throw new Error(intent.error?.code || 'PAYPAL_UNAVAILABLE')
    }

    const { data: topup, error: topupError } = await admin
      .from('topup_requests')
      .insert({ owner_id: user.id, amount: numericAmount, method: 'paypal', reference_number: intent.externalRef, status: 'pending' })
      .select('id')
      .single()
