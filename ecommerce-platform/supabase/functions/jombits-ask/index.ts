...
// sensitive to leak to Groq even though the key itself isn't secret in
// the browser sense (it's called from here, server-side, same as every
// other Vault-backed integration). If disabled, unconfigured, rate-
// limited, or the call fails, the frontend falls back to the existing
// keyword matcher — this function returning an error is an expected,
// handled path, not a bug.
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: credConfig } = await admin.rpc('get_integration_credentials', { p_key: 'ai.groq' })
    if (!credConfig) throw new Error('NOT_CONFIGURED')

    const result = await getAdapter('ai.groq').answer(safeQuestion, { role: safeRole, knowledgeScope: getKnowledgeScope(safeRole) }, credConfig.credentials)

    await admin.rpc('log_integration_event', {
      p_integration_key: 'ai.groq',
