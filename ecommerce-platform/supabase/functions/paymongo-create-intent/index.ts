...
// confirmed a checkout session exists — if PayMongo is unreachable or
// misconfigured, nothing is written and the caller falls back to the
// existing manual (screenshot + reference number) form, unchanged.
Deno.serve(async (req) => {
  ...
  try {
    ...
    const intent = await getAdapter('payments.paymongo').createIntent(
    ...
    if (!intent.ok) {
      ...
      throw new Error(intent.error?.code || 'PAYMONGO_UNAVAILABLE')
    }
    ...
    const { data: topup, error: topupError } = await admin
      ...
      .insert({
        ...
        method: 'paymongo',
        reference_number: intent.externalRef,
        status: 'pending',
      })
      .select('id')
      .single()
    ...
    await admin.from('payment_intents').insert({
      ...
      provider_key: 'payments.paymongo',
    ...
    return new Response(
      JSON.stringify({ checkout_url: intent.redirectUrl, topup_request_id: topup.id }),
