...
// Reseller/Merchant-invoked from TopupModal.jsx's "Pay with Maya" option.
// Mirrors paymongo-create-intent/index.ts exactly (same topup_requests +
// payment_intents insert shape) — only the provider key and adapter differ.
Deno.serve(async (req) => {
  ...
  try {
    ...
    const intent = await getAdapter('payments.maya').createIntent(
    ...
    if (!intent.ok) {
      await admin.rpc('log_integration_event', {
        ...
        p_event_type: 'create_checkout',
        p_status: 'error',
        p_payload: { owner_id: user.id, amount: numericAmount, reference },
        p_error_message: intent.error?.message || 'Unknown error',
      })
      throw new Error(intent.error?.code || 'MAYA_UNAVAILABLE')
    }

    const { data: topup, error: topupError } = await admin
      .from('topup_requests')
      .insert({ owner_id: user.id, amount: numericAmount, method: 'maya', reference_number: intent.externalRef, status: 'pending' })
      .select('id')
      .single()
