...
- Direct SQL against the live database (simulating a user via the `request.jwt.claims` GUC, as an alternative to a real browser session) is an accepted verification method **only for application-level logic** — a `RAISE EXCEPTION` inside a `SECURITY DEFINER` function body that reads `auth.uid()`/`is_admin()`, like `create_voucher`'s `FORBIDDEN` check. It is **not** a valid way to test that an RLS policy or a table `GRANT`/`REVOKE` is actually enforced: `supabase db query --linked` connects as a superuser/owner-equivalent role that bypasses RLS and grants entirely, regardless of what `request.jwt.claims` is set to — that GUC only changes what `auth.uid()` returns, not which Postgres role is executing the query. Confirming a grant was revoked is still reliably done via `information_schema.table_privileges` (a catalog fact, correct regardless of connection role); confirming that revocation actually stops a real request requires a real authenticated PostgREST/browser session. Nor is any of this a substitute for one real browser checkout before trusting checkout-affecting logic in production. As of `TASK6.md` Phase 9/10/12/13, that real-browser pass is still outstanding — flagged there, not silently assumed done.
