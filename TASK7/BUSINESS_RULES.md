...
- **A permissive RLS policy is not a substitute for a role check.** `created_by = auth.uid() or is_admin()` answers "do you own this row," not "are you allowed to create a row shaped like this." Concrete incident: `public.vouchers` originally shipped with direct `insert`/`update`/`delete` grants to `authenticated` and exactly that RLS check — since it never looked at the caller's *role* or the voucher's *scope*, any reseller or customer could have inserted a platform-wide voucher and had it pass. Fixed by revoking the direct grants and forcing every write through a validating `security definer` RPC (`create_voucher`/`update_voucher`/`delete_voucher`, `TASK6.md` Phase 10). When a new table's only real protection is "you must be the owner," check whether *becoming* the owner in the first place needs its own gate.
...
- A suspended or unapproved Merchant's products must not appear to shoppers, even if the product row itself is still `is_active = true` — enforced by `sync_suspended_merchant_products()`, a trigger on `merchant_profiles.status` that force-deactivates the merchant's rows the moment they're suspended/rejected. **This applies to every merchant-owned public listing, not just `products`.** Concrete incident (2026-08-07 security audit): `clinic_services` (referral bookings) never got this same protection when it was added — a suspended merchant could keep creating/editing services, and they stayed publicly bookable indefinitely, because the sync trigger only ever touched `products`. Fixed by hardening `clinic_services`' write policies the same way `products`' already were, and extending the one existing trigger to cover both tables. When adding a new merchant-owned table that shoppers can see, check whether it needs the same suspension wiring `products` has — it won't happen automatically.
- Merchants (and Admin, platform-wide) can create discount **Vouchers** scoped to their whole store, one product, one category, or shipping (`public.vouchers`, `voucher_scope` enum). A voucher's scope/code/merchant/product/category are immutable after creation — price/window/limits can still be edited, or the voucher deactivated, but not repointed at a different target. See `TASK6.md` Phase 10.
...
- A Reseller may apply one voucher code at checkout; it is validated at quote time (`quote_order`, for preview) and re-validated at the moment of purchase (`place_order`, authoritative) so a code that expired or hit its usage limit in between is caught, not silently honored either way.
...
- **RPC-only writes for anything money- or stock-affecting:** direct `UPDATE`/`INSERT` from the client is fine for a user's own simple profile-style data; anything touching balances, fees, stock, or another party's data goes through a `security definer` RPC that re-validates everything server-side. **A table can have the right RPCs and still be vulnerable if the direct table grant from its first migration was never revoked.** Concrete incident (2026-08-07 security audit, `TASKS.md`): `referral_appointments` had `confirm_referral_appointment()`/`complete_referral_appointment()`/`cancel_referral_appointment()` RPCs from day one — correct, and none of them need a client grant since `security definer` runs as the function owner — but the table's original `insert`/`update`/`delete` grant to `authenticated` was never revoked, and its RLS policy had no `WITH CHECK` at all. A merchant could set `status='completed'` via a raw client update, skipping the RPC's wallet transfer entirely and never paying the reseller their referral fee. When adding RPCs to a table, check whether the direct grant should be revoked in the same migration, not left "just in case."
- **Stored notifications (`public.notifications` + `create_notification()`) are for events with no natural source row to derive a feed from.** Most in-app notifications (order status, wallet balance, withdrawal status) are computed live from their own source tables (`RoleNotifications.jsx`/`AdminNotifications.jsx`) — don't write a row to `notifications` for something already visible that way. Reach for `create_notification()` only when there's genuinely no polling table for it, as with a campaign submission's approve/reject or a voucher redemption (`TASK6.md` Phase 12).
