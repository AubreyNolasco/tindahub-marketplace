...
- **Dependency security**: `npm audit fix` applied (brace-expansion, postcss patched). `react-router-dom` upgraded `6.26.2` → `7.18.2` (fixes the open-redirect/constructor-injection advisory). All library-mode APIs only (`Routes`, `Link`, `useNavigate`, etc.) — no breaking changes, build/tests verified green after.
...
- **Nothing has been committed to git yet this session.** All file changes are local working-tree edits (plus the one Supabase migration, which is already live in production — check `git status` before doing anything destructive).
