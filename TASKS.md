...
- **First real payment integration wired up: PayMongo online top-up**, built inside the existing `integration_configs`/`_shared/payments` scaffolding (see integration scaffolding entry above) rather than as a one-off:
  ...
  - **Not yet done**: no real PayMongo credentials exist, so this is unexercised against a live sandbox account. Several payload shapes (webhook event type strings, nested JSON paths) are marked `VERIFY` in the code/docs, matching the same caveat already present on `lalamove-webhook`. Committed, not yet pushed to `origin/main`.

## ⬜ Not started / open decisions

- Remaining integrations from the original prompt (Maya, GCash, Stripe, PayPal, Semaphore SMS, Google Vision OCR, OpenAI, Firebase, SendGrid, Twilio, S3/Cloudinary, Google Maps, Google Analytics, Facebook/Apple login) are NOT built — user asked for "lahat" (all) but agreed to start with one proven end-to-end first. PayMongo is that proof; the rest follow the same adapter-registry recipe once there's a real key to test each against.
- PayMongo needs real sandbox credentials entered via Admin → Settings → Integrations before it can be tested/enabled.
