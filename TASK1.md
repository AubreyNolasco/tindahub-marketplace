...
- [x] **Gemini-backed JOM Bits** — JomBits.jsx's client-side keyword matcher (`jomBitsKnowledge.js`) stays exactly as-is and is the fallback; a real LLM (Google Gemini, free tier — no credit card, per user request for a free option same as Maps→LocationIQ) sits behind it for open-ended phrasing. The model is grounded server-side to a mirrored copy of JOM HUB's own process knowledge (`_shared/ai/knowledge.ts`, KEEP IN SYNC with `jomBitsKnowledge.js`) and explicitly instructed to say "I don't know" rather than guess/invent policy outside it — answers only from "the system," per the user's ask. Never sends account/wallet/order data, only the typed question + role + fixed knowledge text. Resolves the TODO already on file from the scaffolding session: the "does not send your data to a third party" banner in JomBits now reads correctly whether Gemini is on or off. Vault-secret pattern (server-side call, unlike Maps) — `ai.gemini` row added alongside the already-seeded-but-unbuilt `ai.openai`. `20260806000700_gemini_jombits.sql`, `adapters/gemini.ts`, `jombits-ask` edge function, `GEMINI_JOMBITS_SETUP.md`. Needs a real Gemini API key to test; keyword-matcher answers work today regardless.
