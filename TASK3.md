...
- [x] **Batch 4 — Reseller "Customer Orders" inbox.** New `Reseller/StorefrontOrderRequests.jsx` page (list + status filter + detail modal, modeled on `MerchantFollowups.jsx`). Accept/Decline update status directly; Convert re-fetches the full product row, finds-or-creates a `customers` row (matched by phone), calls the existing `useCart().addItem(product, qty, customer, price)`, and sends the Reseller to `/reseller/cart` to finish the unmodified checkout flow. New nav entry (with pending-count badge) in `ResellerLayout.jsx`/`WorkspaceLayout.jsx`. Added a small `sessionStorage`-backed bridge (`utils/storefrontRequestConversion.js`) so `Checkout.jsx` can mark the originating request `converted` (with `converted_customer_id`/`converted_order_id`) as a best-effort follow-up *after* `place_customer_receiver_shipping_order` already succeeds — that RPC itself is untouched.
